...
class PiWebQAIterableDataset(InterleavedBaseIterableDataset):
    def __init__(
        ...
        while True:
            ...
            for row_idx, row in enumerate(data_paths_per_worker_, start=row_start_id):
                try:
                    ...
                    if row_idx == 0:
                        print(f"Task: +++++  {self.dataset_name} {row_idx}  +++++ . Data example: {data['sequence_plan']}")
                    if row_idx <= self.n_log_examples:
                        # Create side-by-side full_example with text
                        self.save_example_image(condition_image, condition_image, prompt, row_idx)
                    data['data_indexes'] = {
